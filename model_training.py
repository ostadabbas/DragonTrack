...
def temporal_loss(current_output, previous_output, reduction='mean'):
    if previous_output is None:  # No temporal loss for the first batch
        return 0
    loss = F.mse_loss(current_output, previous_output, reduction=reduction)
    return loss
...
def model_training(data_list_train, data_list_test, epochs, acc_epoch, acc_epoch2, save_model_epochs, validation_epoch, batchsize, logfilename, load_checkpoint= None):
    ...
    print("Valid Samples List:", len(data_list_test))
    ...
    #train parameters
    weights = [10, 1]
    # Adjusted learning rate upon loading the checkpoint
    optimizer = torch.optim.Adam(complete_net.parameters(), lr=0.001, weight_decay=0.001)

    # Scheduler setup - Example with StepLR
    scheduler = StepLR(optimizer, step_size=4, gamma=0.25)
    ...
    complete_net.train()
    # previous_output = None  # Initialize the previous output
    for epoch in range(initial_epoch, epochs+1):
        ...
        running_loss= 0
        batches_num=0  
        # Reset previous_output at the start of each epoch
        ...
        for batch in trainloader:
            ...
            output, output2, ground_truth, ground_truth2, det_num, tracklet_num, contrastive_loss= complete_net(batch)
            optimizer.zero_grad()
            # loss_temp = temporal_loss(output, previous_output)
...
#             loss = loss + 0.05*(contrastive_loss)
        ...
        if epoch%save_model_epochs==0 and epoch!=0:
            torch.save({ 
                        ...
                        'loss': running_loss,
                        }, './models_v3/epoch_'+str(epoch)+'.pth')
