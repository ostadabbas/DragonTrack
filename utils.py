...
def sinkhorn(matrix): 
    ...
    desired_row_sums = torch.ones((1, row_len), requires_grad=False).to(device)
    desired_col_sums = torch.ones((1, col_len), requires_grad=False).to(device)
    ...
    for _ in range(8):
        ...
        row_normalized_matrix = torch.empty_like(matrix)
        for i in range(row_len):
            row_normalized_matrix[i] = matrix[i] * desired_row_sums[0, i] / actual_row_sum[i]

        # Column normalization
        actual_col_sum = torch.sum(row_normalized_matrix, axis=0)
        col_normalized_matrix = torch.empty_like(matrix)
        for i in range(row_len):
            col_normalized_matrix[i] = row_normalized_matrix[i] * desired_col_sums[0] / actual_col_sum

        matrix = col_normalized_matrix

    return matrix


# def sinkhorn(matrix, tol=1e-5, max_iter=100):
#     row_len, col_len = matrix.shape
#     device = matrix.device
#     epsilon = 1e-8  # Small value to prevent division by zero

#     desired_row_sums = torch.ones(1, row_len, device=device)
#     desired_col_sums = torch.ones(1, col_len, device=device)
#     desired_row_sums[0, -1] = col_len - 1
#     desired_col_sums[0, -1] = row_len - 1

#     for _ in range(max_iter):
#         matrix_old = matrix.clone()

#         # Row normalization
#         actual_row_sum = matrix.sum(dim=1, keepdim=True)
#         matrix = matrix * (desired_row_sums / (actual_row_sum + epsilon))

#         # Column normalization
#         actual_col_sum = matrix.sum(dim=0, keepdim=True)
#         matrix = matrix * (desired_col_sums / (actual_col_sum + epsilon))

#         # Check for convergence
#         if torch.max(torch.abs(matrix - matrix_old)) < tol:
#             break
            ...
            for j, element in enumerate(row):
                if i < a and j < b:
                    ...
                    if p1 == p2 and cnt not in remove_ind:
                        cleaned_output.append(torch.tensor(1, dtype=float).to(device))
                    else:
                        cleaned_output.append(torch.tensor(0, dtype=float).to(device))
                cnt += 1
