...
def data_prep_train(sequence, detections, images_path, frames_look_back, total_frames, most_recent_frame_back, graph_jump, current_frame_train, current_frame_valid, distance_limit, fps, type):
    ...
    while current_frame<=total_frames:
        ...
        # Generate positive and negative pairs based on subject IDs
        positive_pairs, negative_pairs = [], []
        for i, tracklet in enumerate(tracklets):
            for j, detection in enumerate(current_detections):
                # Example to access subject ID, adjust based on your structure
                tracklet_subject_id = tracklet[-1][1]  # Assuming last detection in tracklet represents its ID
                detection_subject_id = detection[1]  # Subject ID of the current detection
                
                if tracklet_subject_id == detection_subject_id:
                    positive_pairs.append((i, j + len(tracklets)))  # Index adjustment for detections
                else:
                    negative_pairs.append((i, j + len(tracklets)))  # Index adjustment for detections
        
        data = build_graph(tracklets, current_detections, images_path, current_frame, distance_limit, fps, positive_pairs, negative_pairs, test=False)
