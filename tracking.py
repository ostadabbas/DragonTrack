...
def get_data(info, set):
    if set=='training':
        detections = np.loadtxt("/home/Galoaa.b/ondemand/data/MOT17/train/MOT17-{}-{}/gt/gt.txt".format(info[0], info[1]), delimiter=',')
        images_path = "/home/Galoaa.b/ondemand/data/MOT17/train/MOT17-{}-{}/img1".format(info[0], info[1])
    elif set=='testing':
        detections = np.loadtxt("/home/Galoaa.b/ondemand/data/MOT17/test/MOT17-{}-{}/det/det.txt".format(info[0], info[1]), delimiter=',')
        images_path = "/home/Galoaa.b/ondemand/data/MOT17/test/MOT17-{}-{}/img1".format(info[0], info[1])
    elif set=='track':
        detections = np.loadtxt("todd_out.txt", delimiter=',')
        images_path = "todd_frames"
...
if __name__ == "__main__":
    ...
    if args.type == 'train':
        ...
        epochs = 20 # at how many epochs to stop
        ...
        acc_epoch2 = validation_epochs # at which epoch to calculate validation accuracy
        ...
        if generate_data:
            ...
            with open('./data/data_train.data', 'wb') as filehandle:
                print("Train Samples List:", len(data_list_train))
            ...
            with open('./data/data_valid.data', 'wb') as filehandle:
                print("Valid Samples List:", len(data_list_valid))
        ...
        match_thres= 0.35 # the matching confidence threshold
...
#         fps= [30,30,14,30,30,30,25]
        ...
        model.load_state_dict(torch.load('./models_v2/epoch_15.pth')['model_state_dict'])
        model.eval()
        print(model)
        ...
        for s in range(len(seq)):
            for d in range(len(detector)):
                detections, images_path = get_data([seq[s], detector[d]], "track")
...
#                 print('Sequence: '+ seq[s])
                ...
                #write output
                outputFile = './output_v2/todd-{}-{}.avi'.format(seq[s],detector[d])
                #get dimensions of images
                image_name = os.path.join(images_path, "{0:0=4d}".format(1) + ".jpg")
                image = cv2.imread(image_name, cv2.IMREAD_COLOR)
                vid_writer = cv2.VideoWriter(outputFile, cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), 15,
                                             (image.shape[1], image.shape[0]))
                with open('./output_v2/todd-{}-{}.txt'.format(seq[s],detector[d]), 'w') as f:
                    for frame in range(1,int(total_frames)+1):
                        image_name = os.path.join(images_path, "{0:0=4d}".format(int(frame)) + ".jpg")
                        image = cv2.imread(image_name, cv2.IMREAD_COLOR)
